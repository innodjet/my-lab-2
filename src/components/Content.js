...
class Content extends Component {
  ...
  page = data => {
    let result;
    if (data.length % 4 === 0) {
      result = parseInt(data.length / 4);
    } else {
      result = parseInt(data.length / 4) + 1;
    }
    return result;
  };
  ...
  next = () => {
    let reqPage =
      parseInt(this.state.pageSlected) + 1 < this.state.totalPage
        ? parseInt(this.state.pageSlected) + 1
      ...
      .then(response => {
        if (response.status === 200) {
          ...
          this.setState({
            message: {
              type: 1,
              desc: "Your contact have been successfully added"
            }
          });
          this.getContactData();
        }
  ...
  confirmEditContact = (name, email, phone, id) => {
    ...
    let data = { name: name, email: email, phone: phone, id: id };
    fetch(url, {
      method: "PUT",
      body: JSON.stringify(data),
      headers: {
      ...
      .then(response => {
        if (response.status === 200) {
          ...
          this.setState({
            message: {
              type: 1,
              desc: "Your contact have been successfully edited"
            }
          });
          this.getContactData();
        }
  ...
  validate = e => {
    ...
    if (nameValid) {
      ...
      this.setState({
        isNameValid: { rep: nameValid, style: { border: "1px solid #ba2629" } }
      });
    }
    ...
    if (emailValid) {
      this.setState({
        isEmailValid: { rep: emailValid, style: { border: "" } }
      });
    } else {
      this.setState({
        isEmailValid: {
          rep: emailValid,
          style: { border: "1px solid #ba2629" }
        }
      });
    }
    ...
    if (phoneValid) {
      this.setState({
        isPhoneValid: { rep: phoneValid, style: { border: "" } }
      });
    } else {
      this.setState({
        isPhoneValid: {
    ...
    if (nameValid && emailValid && phoneValid) {
      ...
      if (this.state.editMode.rep) {
        ...
        // Adding new contact to the existing list
      ...
      this.setState({
        message: {
          type: 0,
          desc: "Your form is invalid , please check field(s) for error(s)"
        }
      });
    }
  };
      ...
      .then(response => {
        if (response.status === 200) {
          ...
          this.setState({
            message: {
              type: 1,
              desc: "Your contact have been deleted successfully"
            }
          });
          this.getContactData();
        }
  ...
  editContact = async id => {
    ...
    const { name, email, phone } = await this.state.data.filter(
      el => el.id === id
    )[0];
    this.setState({
      name: name,
  ...
  render() {
    ...
    const submitButttontextValue =
      this.state.editMode.rep === false ? "Add Contact" : "Edit Contact";
    const modelDelButStyle = { width: "120px" };
    const modelCancelButStyle = {
      width: "120px",
        ...
        <div className="container">
          <div className="row" style={tabStyle}>
            ...
            <div className="col-sm-6">
              <Contacts
                contacts={this.state.dataToDisplay}
